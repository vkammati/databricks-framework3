...
dv_workflow_suffix = "DV-"+args.deployment_type
dv_workflow_name = get_wrapping_workflow_name(args.repository, args.branch_name, dv_workflow_suffix)

uom_workflow_suffix = "UOM-"+args.deployment_type
uom_workflow_name = get_wrapping_workflow_name(args.repository, args.branch_name, uom_workflow_suffix)

uom_demo_workflow_suffix = "UOM-DEMO-"+args.deployment_type
uom_demo_workflow_name = get_wrapping_workflow_name(args.repository, args.branch_name, uom_demo_workflow_suffix)
...
dv_unique_repo_branch_id = get_unique_repo_branch_id(args.repository, args.branch_name, dv_workflow_suffix ) if args.env == "dev" else ""

uom_unique_repo_branch_id = get_unique_repo_branch_id(args.repository, args.branch_name, uom_workflow_suffix ) if args.env == "dev" else ""

uom_demo_unique_repo_branch_id = get_unique_repo_branch_id(args.repository, args.branch_name, uom_demo_workflow_suffix ) if args.env == "dev" else ""
...
dv_reset_uc_and_adls_conf_filename = "dv_reset_uc_and_adls_workflow.json"
dv_reset_uc_and_adls_conf_template = common_templates.get_template(dv_reset_uc_and_adls_conf_filename)
uom_reset_uc_and_adls_conf_filename = "uom_reset_uc_and_adls_workflow.json"
uom_reset_uc_and_adls_conf_template = common_templates.get_template(uom_reset_uc_and_adls_conf_filename)
uom_demo_reset_uc_and_adls_conf_filename = "uom_demo_reset_uc_and_adls_workflow.json"
uom_demo_reset_uc_and_adls_conf_template = common_templates.get_template(uom_demo_reset_uc_and_adls_conf_filename)
...
dv_reset_uc_and_adls_conf_str = dv_reset_uc_and_adls_conf_template.render(
    repos_path=repos_path,
    unique_repo_branch_id=dv_unique_repo_branch_id,
    unique_repo_branch_id_schema=dv_unique_repo_branch_id,
    dlt_pipeline_id=dlt_pipeline_id if dlt_pipeline_id else "",
    env=args.env,
    timeout=args.timeout,
    run_name=f"reset-{args.env}-{dv_unique_repo_branch_id}",
)

uom_reset_uc_and_adls_conf_str = uom_reset_uc_and_adls_conf_template.render(
    repos_path=repos_path,
    unique_repo_branch_id=uom_unique_repo_branch_id,
    unique_repo_branch_id_schema=uom_unique_repo_branch_id,
    dlt_pipeline_id=dlt_pipeline_id if dlt_pipeline_id else "",
    env=args.env,
    timeout=args.timeout,
    run_name=f"reset-{args.env}-{uom_unique_repo_branch_id}",
)

uom_demo_reset_uc_and_adls_conf_str = uom_demo_reset_uc_and_adls_conf_template.render(
    repos_path=repos_path,
    unique_repo_branch_id=uom_demo_unique_repo_branch_id,
    unique_repo_branch_id_schema=uom_demo_unique_repo_branch_id,
    dlt_pipeline_id=dlt_pipeline_id if dlt_pipeline_id else "",
    env=args.env,
    timeout=args.timeout,
    run_name=f"reset-{args.env}-{uom_demo_unique_repo_branch_id}",
)
...
dv_reset_uc_and_adls_conf = json.loads(dv_reset_uc_and_adls_conf_str)
dv_reset_uc_and_adls_workflow_run_id = trigger_one_time_run(dv_reset_uc_and_adls_conf, args.host, http_header)
print(f"New job run to delete UC and ADLS folders has been triggered with id {dv_reset_uc_and_adls_workflow_run_id}.")

uom_reset_uc_and_adls_conf = json.loads(uom_reset_uc_and_adls_conf_str)
uom_reset_uc_and_adls_workflow_run_id = trigger_one_time_run(uom_reset_uc_and_adls_conf, args.host, http_header)
print(f"New job run to delete UC and ADLS folders has been triggered with id {uom_reset_uc_and_adls_workflow_run_id}.")

uom_demo_reset_uc_and_adls_conf = json.loads(uom_demo_reset_uc_and_adls_conf_str)
uom_demo_reset_uc_and_adls_workflow_run_id = trigger_one_time_run(uom_demo_reset_uc_and_adls_conf, args.host, http_header)
print(f"New job run to delete UC and ADLS folders has been triggered with id {uom_demo_reset_uc_and_adls_workflow_run_id}.")

run_status = poll_run(reset_uc_and_adls_workflow_run_id, args.host, http_header)
dv_run_status = poll_run(dv_reset_uc_and_adls_workflow_run_id, args.host, http_header)
uom_run_status = poll_run(uom_reset_uc_and_adls_workflow_run_id, args.host, http_header)
uom_demo_run_status = poll_run(uom_demo_reset_uc_and_adls_workflow_run_id, args.host, http_header)

if run_status != "SUCCESS" or dv_run_status != "SUCCESS" or uom_run_status != "SUCCESS" or uom_demo_run_status != "SUCCESS":
    sys.exit(1)
...
if args.env == "dev":
    ...
    delete_workflow_by_name(dv_workflow_name, args.host, http_header) 
    delete_workflow_by_name(uom_workflow_name, args.host, http_header)
    delete_workflow_by_name(uom_demo_workflow_name, args.host, http_header)
